...
class Offers extends Component
{
    ...
    public function saveGroup()
    {
        ...
        if($this->updateMode){
            ...
            $Groups->save();
            // حفظ العلاقة
            $Groups->pivot_service_group()->detach();
            foreach ($this->GroupsItems as $GroupsItem) {
                $Groups->pivot_service_group()->attach($GroupsItem['service_id']);
            }
            ...
            // حفظ العلاقة
            foreach ($this->GroupsItems as $GroupsItem) {
                $Groups->pivot_service_group()->attach($GroupsItem['service_id'],['quantity' => $GroupsItem['quantity']]);
    ...
    public function edit($id)
    {
        ...
        foreach ($group->pivot_service_group as $serviceGroup)
        {
            $this->GroupsItems[] = [
